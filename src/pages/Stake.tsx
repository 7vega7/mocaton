...
const LOCK_TYPES = [
  { id: 'flexible', label: 'Flexible', desc: 'Cancel kapanpun\n(tunggu 24 jam)', rate: 100, color: '#4CAF50' },
  { id: 'weekly', label: '1 Minggu', desc: 'Lock 7 hari\nBonus +20%', rate: 120, color: '#2196F3' },
  { id: 'monthly', label: '1 Bulan', desc: 'Lock 30 hari\nBonus +60%', rate: 160, color: '#9C27B0' },
];
...
export default function Stake({ user }: StakePageProps) {
  ...
  const handleStake = async () => {
    if (!amount || parseFloat(amount) <= 0) { setError('Masukkan jumlah TON yang valid'); return; }
    if (!userAddress) { tonConnectUI.openModal(); return; }
    setIsStaking(true);
    setError('');
    try {
      const amountNano = BigInt(Math.floor(parseFloat(amount) * 1e9)).toString();
      const transaction = {
      ...
      const res = await fetch('/api/stake/create', {
        ...
        body: JSON.stringify({ amount_ton: parseFloat(amount), lock_type: lockType, tx_hash: result.boc, ton_wallet: userAddress }),
      });
      ...
      setError(err.message?.includes('User rejects') ? 'Transaksi dibatalkan' : err.message || 'Terjadi kesalahan');
    } finally {
      setIsStaking(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="stake-success">
        ...
        <p>Poin mulai berjalan sekarang.</p>
        <button className="btn-primary" onClick={() => setSuccess(false)}>Stake Lagi</button>
      </div>
    );
  }
    ...
    <div className="stake-page">
      ...
      {!userAddress ? (
        <div className="wallet-connect-banner">
          ...
          <button className="btn-primary" onClick={() => tonConnectUI.openModal()}>Hubungkan Wallet</button>
        </div>
      ) : (
      ...
      {parseFloat(amount) > 0 && (
        <div className="estimate-card">
          ...
          <div className="estimate-row"><span>Per Hari</span><strong>{(parseFloat(amount) * selectedRate).toLocaleString('id-ID')} poin</strong></div>
          <div className="estimate-row"><span>Per Minggu</span><strong>{(parseFloat(amount) * selectedRate * 7).toLocaleString('id-ID')} poin</strong></div>
          <div className="estimate-row"><span>Per Bulan</span><strong>{(parseFloat(amount) * selectedRate * 30).toLocaleString('id-ID')} poin</strong></div>
        </div>
      )}
